"use client"; import { useRef, useEffect } from "react"; export type PaintFn = (ctx: CanvasRenderingContext2D, w: number, h: number, freqData: Uint8Array | null,) => void;
...
export function useCanvasLoop(
  ...
  const canvasRef = useRef<HTMLCanvasElement>(null); const frameRef = useRef(0); const paintRef = useRef(paint); const freqRef = useRef(frequencyDataRef); const sizeRef = useRef({ w: 0, h: 0 }); useEffect(() => { paintRef.current = paint; }); useEffect(() => { freqRef.current = frequencyDataRef; }, [frequencyDataRef]); useEffect(() => { const canvas = canvasRef.current; if (!canvas) return; const updateSize = () => { // Track canvas size via ResizeObserver instead of getBoundingClientRect() per frame
