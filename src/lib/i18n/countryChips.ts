...
function getSameLanguageCountries(locale: SupportedLocale): string[] {
  ...
  if (seedCodes.length === 0) return []; const seed = seedCodes .map((code) => COUNTRY_BY_CODE[code]).filter(Boolean); const regions = new Set(seed.map((country) => country.region)); const subregions = new Set(seed.map((country) => country.subregion));
...
export function getCountryChipsForLocale(locale: SupportedLocale, maxChips = 36): CountryChip[] {
  ...
  uniquePush(ordered, proximityCodes); uniquePush(ordered, GLOBAL_INTEREST_CODES); const capped = ordered .filter((code) => COUNTRY_BY_CODE[code] && !EXCLUDED_LOW_RELEVANCE_CODES.has(code)).slice(0, maxChips); const languageSet = new Set(languageCodes); const proximitySet = new Set(proximityCodes); return capped.map((code) => {
