...
import { NextRequest, NextResponse } from 'next/server'; export const runtime = 'nodejs'; const MB_BASE = 'https://musicbrainz.org/ws/2'; const WIKI_BASE = 'https://en.wikipedia.org/api/rest_v1'; const USER_AGENT = 'PulseRadio/1.0 (https://pulse-radio.online)'; const MUSIC_KEYWORDS =
...
async function fetchJson<T>(url: string, headers: Record<string, string>): Promise<T | null> { try {
    const res = await fetch(url, { headers, signal: AbortSignal.timeout(8_000) }); if (!res.ok) { await res.text().catch(() => {}); return null; } const cl = res.headers.get('content-length'); if (cl && parseInt(cl, 10) > 2 * 1024 * 1024) { await res.body?.cancel().catch(() => {}); return null; } return await res.json();
  } catch { return null; } }
async function searchMusicBrainz(artist: string) {
  ...
  const data = await fetchJson<{ artists?: any[] }>(url, { 'User-Agent': USER_AGENT, Accept: 'application/json' }); return data?.artists?.[0] ?? null; }
async function fetchWikiSummary(title: string) { const url = `${WIKI_BASE}/page/summary/${encodeURIComponent(title)}`; const data = await fetchJson<any>(url, { 'User-Agent': USER_AGENT }); // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function GET(req: NextRequest) { const artist = req.nextUrl.searchParams.get('artist'); if (!artist || artist.length > 200) {
  ...
  try { const [mbResult, wikiResult] = await Promise.allSettled([ searchMusicBrainz(artist), fetchWikiSummary(artist), ]); const mb = mbResult.status === 'fulfilled' ? mbResult.value : null; let wiki = wikiResult.status === 'fulfilled' ? wikiResult.value : null;
    ...
    const hasData = !!(mb || wiki?.extract); const cacheHeader = hasData ? 'public, max-age=86400, stale-while-revalidate=604800' : 'public, max-age=3600, stale-while-revalidate=7200'; return NextResponse.json( { name: mb?.name ?? artist, disambiguation: mb?.disambiguation ?? null, type: mb?.type ?? null, country: mb?.country ?? null,
