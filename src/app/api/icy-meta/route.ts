...
export async function GET(req: NextRequest) { const streamUrl = req.nextUrl.searchParams.get('url'); if (!streamUrl || streamUrl.length > 2048) {
  ...
  try { const url = new URL(streamUrl); if (!['http:', 'https:'].includes(url.protocol)) {
    ...
    if (res.url) { try { const finalUrl = new URL(res.url); if (isPrivateHost(finalUrl.hostname.toLowerCase())) { clearTimeout(timeout); res.body?.cancel().catch(() => {}); return NextResponse.json({ error: 'Redirect to private IP not allowed' }, { status: 403 }); } } catch { /* URL parse failed — continue */ } }
    ...
    const reader = res.body.getReader(); const chunks: Uint8Array[] = []; let totalRead = 0; const bytesNeeded = metaint + 4096; try { while (totalRead < bytesNeeded) { const { done, value } = await reader.read(); if (done || !value) break; chunks.push(value); totalRead += value.length; }
