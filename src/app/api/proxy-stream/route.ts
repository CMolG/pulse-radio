...
import { NextRequest } from 'next/server'; import { isPrivateHost } from '@/lib/urlSecurity'; export const runtime = 'nodejs'; const ALLOWED_PROTOCOLS = ['http:', 'https:']; const MAX_DURATION_MS = 0; // 0 = no forced timeout; stream should run indefinitely
/* Proxies an internet radio stream, adding CORS headers so the browser can use it with <audio crossOrigin="anony
 * mous"> + Web Audio API. */
export async function GET(req: NextRequest) { const streamUrl = req.nextUrl.searchParams.get('url'); if (!streamUrl || streamUrl.length > 2048) { return new Response(JSON.stringify({ error: 'Missing or invalid url parameter' }), { status: 400, headers: { 'Content-Type': 'application/json' },});
  } let parsed: URL; try { parsed = new URL(streamUrl); if (!ALLOWED_PROTOCOLS.includes(parsed.protocol)) { return new Response(JSON.stringify({ error: 'Invalid protocol' }), { status: 400, headers: { 'Content-Type': 'application/json' },});
    } const host = parsed.hostname.toLowerCase(); // Block loopback and private/internal IPs to prevent SSRF
    if (isPrivateHost(host)) { return new Response(JSON.stringify({ error: 'Private/internal URLs not allowed' }), { status: 400, headers: { 'Content-Type': 'application/json' },}); }
    ...
    if (upstream.url) { try { const finalUrl = new URL(upstream.url); if (isPrivateHost(finalUrl.hostname.toLowerCase())) { // Validate the final URL after redirects to prevent SSRF via redirect
          if (timeout) clearTimeout(timeout); upstream.body?.cancel().catch(() => {}); return new Response(JSON.stringify({ error: 'Redirect to private IP not allowed' }), { status: 403, headers: { 'Content-Type': 'application/json' },});
        }} catch {
    ...
    if (!upstream.ok || !upstream.body) { if (timeout) clearTimeout(timeout); upstream.body?.cancel().catch(() => {}); // release connection
      return new Response(JSON.stringify({ error: `Upstream ${upstream.status}` }), { status: 502, headers: { 'Content-Type': 'application/json', 'Retry-After': '3' },});
  ...
  } catch (err) { if (timeout) clearTimeout(timeout); const isTimeout = err instanceof DOMException && err.name === 'AbortError'; if (isTimeout) { return new Response(JSON.stringify({ error: 'Stream timed out' }), { status: 504, headers: { 'Content-Type': 'application/json' },});
    } const message = err instanceof Error ? err.message : 'Unknown error'; return new Response(JSON.stringify({ error: message }), { status: 502, headers: { 'Content-Type': 'application/json', 'Retry-After': '5' },});
  } }
