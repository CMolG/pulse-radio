...
export async function GET(req: NextRequest) { const term = req.nextUrl.searchParams.get('term'); if (!term || term.length > 200) {
  ...
  const media = req.nextUrl.searchParams.get('media') === 'podcast' ? 'podcast' : 'music'; const entity = media === 'podcast' ? 'podcast' : 'song'; const limit = media === 'podcast' ? '20' : '3';
  try { const url = `https://itunes.apple.com/search?${new URLSearchParams({ term, media, entity, limit, })}`; const res = await apiFetch(url, { timeoutMs: 8_000, maxBytes: 2 * 1024 * 1024, label: 'iTunes API' }); const data = await res.json();
