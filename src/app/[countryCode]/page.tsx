...
export async function generateMetadata({ params }: CountryPageProps): Promise<Metadata> {
  ...
  const country = COUNTRY_BY_CODE[countryCode]; if (!country) return {}; const title = `Top radio stations in ${country.name} (${countryCode})`;
...
export default async function CountryPage({ params }: CountryPageProps) {
  const resolved = await params; const countryCode = normalizeCountryCode(resolved.countryCode); if (!COUNTRY_BY_CODE[countryCode]) notFound();
  return ( <div className="h-full w-full"><Radio initialCountryCode={countryCode} /></div> ); }
