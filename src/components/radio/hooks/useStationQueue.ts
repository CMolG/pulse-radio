...
export function useStationQueue() { const [queue, setQueue] = useState<Station[]>(() => loadFromStorage<Station[]>(STORAGE_KEY, [])
  ...
  useEffect(() => { if (persistRef.current) saveToStorage(STORAGE_KEY, queue); persistRef.current = true; }, [queue]);
  const add = useCallback((station: Station) => { setQueue(prev => {
      ...
      let adjusted = prevIdx; if (removedIdx >= 0 && removedIdx < prevIdx) adjusted--;
  ...
  }, []); const moveDown = useCallback((stationuuid: string) => { let movedIdx = -1;
    setQueue(prev => { const idx = prev.findIndex(s => s.stationuuid === stationuuid);
      if (idx < 0 || idx >= prev.length - 1) return prev; movedIdx = idx; const next = [...prev];
      [next[idx], next[idx + 1]] = [next[idx + 1], next[idx]]; return next;
    }); setCurrentIndex(prev => { if (movedIdx < 0) return prev; if (prev === movedIdx) return movedIdx + 1;
      if (prev === movedIdx + 1) return movedIdx; return prev;});}, []);
