...
'use client'; import { useState, useCallback, useRef, useEffect } from 'react'; import type { Station } from '../types'; import { loadFromStorage, saveToStorage } from '@/lib/storageUtils'; const STORAGE_KEY = 'radio-station-queue'; const MAX_QUEUE_SIZE = 20; export function useStationQueue() { const [queue, setQueue] = useState<Station[]>(() => loadFromStorage<Station[]>(STORAGE_KEY, []) ); const [currentIndex, setCurrentIndex] = useState(-1); const persistRef = useRef(false);
  ...
  }, []); const remove = useCallback((stationuuid: string) => { let removedIdx = -1; setQueue(prev => { removedIdx = prev.findIndex(s => s.stationuuid === stationuuid); return prev.filter(s => s.stationuuid !== stationuuid); }); setCurrentIndex(prev => { if (removedIdx < 0) return prev; if (removedIdx < prev) return prev - 1; if (removedIdx === prev) return -1; return prev;}); }, []); const clear = useCallback(() => { setQueue([]); setCurrentIndex(-1); }, []); const moveUp = useCallback((stationuuid: string) => { let movedIdx = -1; setQueue(prev => { const idx = prev.findIndex(s => s.stationuuid === stationuuid); if (idx <= 0) return prev; movedIdx = idx; const next = [...prev]; [next[idx - 1], next[idx]] = [next[idx], next[idx - 1]]; return next; }); setCurrentIndex(prev => { if (movedIdx < 0) return prev; if (prev === movedIdx) return movedIdx - 1; if (prev === movedIdx - 1) return movedIdx; return prev;}); }, []); const moveDown = useCallback((stationuuid: string) => { let movedIdx = -1; setQueue(prev => { const idx = prev.findIndex(s => s.stationuuid === stationuuid); if (idx < 0 || idx >= prev.length - 1) return prev; movedIdx = idx; const next = [...prev]; [next[idx], next[idx + 1]] = [next[idx + 1], next[idx]]; return next; }); setCurrentIndex(prev => { if (movedIdx < 0) return prev; if (prev === movedIdx) return movedIdx + 1; if (prev === movedIdx + 1) return movedIdx; return prev;});}, []); const skipToNext = useCallback((): Station | null => {
