...
export type StreamQuality = 'good' | 'fair' | 'poor' | 'offline'; export type StreamLatency = { url: string; latencyMs: number; timestamp: number; }; export function useRadio() { const audioRef = useRef<HTMLAudioElement | null>(null); const retryRef = useRef(0); const fadeTimerRef = useRef<ReturnType<typeof setInterval> | null>(null); const pauseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null); const reconnectTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null); const bufferCheckRef = useRef<ReturnType<typeof setInterval> | null>(null); const playSessionRef = useRef(0); const proxyFallbackUrlsRef = useRef<Set<string>>(new Set()); const codecFallbackTriedRef = useRef<Set<string>>(new Set()); const isReconnectingRef = useRef(false); // Gate: prevents concurrent reconnect attempts (cleared on success/failure)
  ...
  const bcRef = useRef<BroadcastChannel | null>(null); const tabIdRef = useRef(`${Date.now()}-${Math.random().toString(36).slice(2)}`); useEffect(() => { if (typeof BroadcastChannel === 'undefined') return; const bc = new BroadcastChannel('pulse-radio-playback'); bcRef.current = bc; bc.onmessage = (e: MessageEvent) => { if (e.data?.type === 'playing' && e.data?.tabId !== tabIdRef.current) {
        ...
        clearTimer(fadeTimerRef); } // calls startPlayback() and overrides the pause from the other tab. // after the cross-tab pause. Without this, the crossfade completion
  ...
  const handlePlayRejected = useCallback((err: unknown) => {
    ...
    if (err instanceof DOMException && err.name === 'AbortError') return; setStatus(isAutoplayBlocked(err) ? 'paused' : 'error');}, []); // already set the correct status (idle/loading), so swallow silently.
  const startPlayback = useCallback(
    (audio: HTMLAudioElement, streamUrl: string, onRejected: (err: unknown) => void,) => {
      ...
      const webAudioConnected = hasAudioSource(audio); const shouldUseProxy = !preferDirectStream || proxyFallbackUrlsRef.current.has(streamUrl) || webAudioConnected; const setSourceAndPlay = (useProxy: boolean) => { // the response has CORS headers so the Web Audio pipeline outputs sound.
      ...
      setSourceAndPlay(shouldUseProxy).catch((err) => {
        ...
        if (!shouldUseProxy && preferDirectStream && !isAutoplayBlocked(err)) { // If direct fails for non-autoplay reasons, fallback to proxy for this station.
      ...
      const jitter = adaptedDelay * (0.7 + Math.random() * 0.6); reconnectTimerRef.current = setTimeout(() => { reconnectTimerRef.current = null; if (userPausedRef.current || playSessionRef.current !== sessionId) { isReconnectingRef.current = false; return; } startPlayback(audio, station.url_resolved, (err) => {
    ...
    const onPause = () => {
      ...
      if (srcChangingRef.current) return; // treat this as an OS interruption and queue a spurious reconnect. // The browser fires a synchronous 'pause' event when src changes; we must not
      ...
      if (!station) return; setStatus('loading'); clearTimer(pauseTimerRef); pauseTimerRef.current = setTimeout(() => { if (userPausedRef.current || !audio.paused) return; audio.play().catch((err) => { if (isAutoplayBlocked(err)) {
        ...
        if (station && !codecFallbackTriedRef.current.has(station.url_resolved)) { // Only attempt this fallback once per station to avoid loops.
    ...
    let stallCount = 0; const onStalled = () => { clearTimer(stallTimerRef); stallCount++; let bufferAhead = 0; if (audio.buffered.length > 0) bufferAhead = audio.buffered.end(audio.buffered.length - 1) - audio.currentTime;
      ...
      const baseTimeout = bufferAhead <= 0 ? 1000 : bufferAhead < 2 ? 2000 : 6000; const timeout = Math.max(500, baseTimeout / Math.min(stallCount, 4)); stallTimerRef.current = setTimeout(() => { stallTimerRef.current = null; if (audio.paused || audio.readyState < 3) {
    ...
    const onEnded = () => { if (!userPausedRef.current && station) reconnect(500); }; const onTimeUpdate = () => setCurrentTime(audio.currentTime); const onCanPlay = () => { if (!userPausedRef.current && station && audio.paused) audio.play().catch(() => {}); };
    ...
    let lastResumeAttempt = 0; const RESUME_DEBOUNCE_MS = 1000; const onVisibilityResume = () => { // so without debouncing we get two parallel reconnect attempts.
    ...
    const BUFFER_CHECK_MS = 2000; const MIN_BUFFER_AHEAD_S = 2; let lowBufferStreak = 0; clearTimer(bufferCheckRef); bufferCheckRef.current = setInterval(() => { // Also measures stream quality based on buffer growth rate.
      ...
      lastBufferEndRef.current = bufferEnd; if (ahead >= 5) {
        ...
        if (lowBufferStreak >= 2) { lowBufferStreak = 0; reconnect(300); } // from momentary dips during normal streaming
    ...
    isReconnectingRef.current = false; proxyFallbackUrlsRef.current.delete(s.url_resolved); codecFallbackTriedRef.current.delete(s.url_resolved); setStation(s); setStatus('loading'); setStreamQuality('good'); lastBufferEndRef.current = 0;
