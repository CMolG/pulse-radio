...
export function useEqualizer() { const [bands, setBands] = useState<EqBand[]>(() => {
  ...
  ); const [normalizerEnabled, setNormalizerEnabled] = useState(getDefaultNormalizerEnabled); const [stereoWidth, setStereoWidthState] = useState(() =>loadFromStorage<number>(STORAGE_KEYS.STEREO_WIDTH, 1.0) );
  const [bassEnhance, setBassEnhanceState] = useState(() =>loadFromStorage<number>(STORAGE_KEYS.BASS_ENHANCE, 0) ); const [compressorEnabled, setCompressorEnabled] = useState(() => loadFromStorage<boolean>(STORAGE_KEYS.COMPRESSOR_ENABLED, false));
  const [compressorAmount, setCompressorAmountState] = useState(() => loadFromStorage<number>(STORAGE_KEYS.COMPRESSOR_AMOUNT, 0.5)); const [noiseReductionMode, setNoiseReductionModeState] = useState<NoiseReductionMode>(getDefaultNoiseReductionMode);
  const ctxRef = useRef<AudioContext | null>(null); const sourceRef = useRef<MediaElementAudioSourceNode | null>(null); const filtersRef = useRef<BiquadFilterNode[]>([]); const limiterRef = useRef<DynamicsCompressorNode | null>(null);
  ...
  const directGainLRef = useRef<GainNode | null>(null); const directGainRRef = useRef<GainNode | null>(null); const crossGainLRef = useRef<GainNode | null>(null); const crossGainRRef = useRef<GainNode | null>(null);
  const bassLpRef = useRef<BiquadFilterNode | null>(null); const bassShaperRef = useRef<WaveShaperNode | null>(null); const bassHpRef = useRef<BiquadFilterNode | null>(null); const bassMixRef = useRef<GainNode | null>(null);
  const nrHighpassRef = useRef<BiquadFilterNode | null>(null); const nrGateRef = useRef<DynamicsCompressorNode | null>(null); const nrDeEsserRef = useRef<BiquadFilterNode | null>(null); const nrDeEssGainRef = useRef<GainNode | null>(null);
  // Multiband compressor refs: 3 bands (low/mid/high) with crossover filters
  const mbLowLpRef = useRef<BiquadFilterNode | null>(null); const mbLowCompRef = useRef<DynamicsCompressorNode | null>(null); const mbMidBpLpRef = useRef<BiquadFilterNode | null>(null); const mbMidBpHpRef = useRef<BiquadFilterNode | null>(null);
  const mbMidCompRef = useRef<DynamicsCompressorNode | null>(null); const mbHighHpRef = useRef<BiquadFilterNode | null>(null); const mbHighCompRef = useRef<DynamicsCompressorNode | null>(null); const mbDryGainRef = useRef<GainNode | null>(null);
  ...
  const graphNodeRefs: React.MutableRefObject<AudioNode | null>[] = [
    normalizerRef, normGainRef, limiterRef, splitterRef, mergerRef, outputGainRef, directGainLRef, directGainRRef, crossGainLRef, crossGainRRef, mbLowLpRef, mbLowCompRef, mbMidBpLpRef, mbMidBpHpRef, mbMidCompRef,
  ...
  const applyNoiseReductionPreset = useCallback((mode: NoiseReductionMode) => {
    const preset = NR_PRESETS[mode]; const ctx = ctxRef.current; const t = ctx?.currentTime ?? 0; if (nrHighpassRef.current) nrHighpassRef.current.frequency.setTargetAtTime(preset.hpfHz, t, RAMP_TIME);
  ...
  const toggleCompressor = useCallback(() => { setCompressorEnabled(prev => {
    ...
    const clamped = Math.max(0, Math.min(1, v)); setCompressorAmountState(clamped); saveToStorage(STORAGE_KEYS.COMPRESSOR_AMOUNT, clamped); if (!compressorEnabled) return; const ctx = ctxRef.current; const t = ctx?.currentTime ?? 0;
