...
const QUALITY_DEFAULTS_MIGRATION_KEY = 'radio-quality-defaults-v2-applied'; function ensureQualityMigration(): void { if (loadFromStorage<boolean>(QUALITY_DEFAULTS_MIGRATION_KEY, false)) return;
  saveToStorage(STORAGE_KEYS.NOISE_REDUCTION_MODE, 'low'); saveToStorage(STORAGE_KEYS.NORMALIZER_ENABLED, true); saveToStorage(QUALITY_DEFAULTS_MIGRATION_KEY, true); }
...
export function useEqualizer() { const [bands, setBands] = useState<EqBand[]>(() => {
  ...
  }); const [enabled, setEnabled] = useState(true); const [customPresets, setCustomPresets] = useState<EqPreset[]>(() => loadFromStorage<EqPreset[]>(STORAGE_KEYS.CUSTOM_EQ_PRESETS, [])
  ...
  const [compressorEnabled, setCompressorEnabled] = useState(() => loadFromStorage<boolean>(STORAGE_KEYS.COMPRESSOR_ENABLED, false));
  const [compressorAmount, setCompressorAmountState] = useState(() => loadFromStorage<number>(STORAGE_KEYS.COMPRESSOR_AMOUNT, 0.5));
  ...
  const normalizerRef = useRef<DynamicsCompressorNode | null>(null); const normGainRef = useRef<GainNode | null>(null); const splitterRef = useRef<ChannelSplitterNode | null>(null);
  const mergerRef = useRef<ChannelMergerNode | null>(null); const outputGainRef = useRef<GainNode | null>(null); const outputVolumeRef = useRef(1); const outputMutedRef = useRef(false);
  ...
  const nrHighpassRef = useRef<BiquadFilterNode | null>(null); const nrGateRef = useRef<DynamicsCompressorNode | null>(null);
  ...
  const mbLowLpRef = useRef<BiquadFilterNode | null>(null); const mbLowCompRef = useRef<DynamicsCompressorNode | null>(null);
  const mbMidBpLpRef = useRef<BiquadFilterNode | null>(null); const mbMidBpHpRef = useRef<BiquadFilterNode | null>(null);
  const mbMidCompRef = useRef<DynamicsCompressorNode | null>(null); const mbHighHpRef = useRef<BiquadFilterNode | null>(null);
  const mbHighCompRef = useRef<DynamicsCompressorNode | null>(null); const mbDryGainRef = useRef<GainNode | null>(null);
  const mbWetGainRef = useRef<GainNode | null>(null); const mbMergeRef = useRef<GainNode | null>(null); const connectedAudioRef = useRef<HTMLAudioElement | null>(null);
  ...
  const graphNodeRefs: React.MutableRefObject<AudioNode | null>[] = [
    ...
    directGainLRef, directGainRRef, crossGainLRef, crossGainRRef, mbLowLpRef, mbLowCompRef, mbMidBpLpRef, mbMidBpHpRef, mbMidCompRef,
    mbHighHpRef, mbHighCompRef, mbDryGainRef, mbWetGainRef, mbMergeRef, nrHighpassRef, nrGateRef, nrDeEsserRef, nrDeEssGainRef, bassLpRef, bassShaperRef, bassHpRef, bassMixRef,];
  ...
  const applyNoiseReductionPreset = useCallback((mode: NoiseReductionMode) => {
    ...
    if (nrGateRef.current) { nrGateRef.current.threshold.setTargetAtTime(preset.gateThreshold, t, RAMP_TIME); nrGateRef.current.ratio.setTargetAtTime(preset.gateRatio, t, RAMP_TIME); }
    if (nrDeEsserRef.current) { nrDeEsserRef.current.frequency.setTargetAtTime(preset.deEsserCenterHz, t, RAMP_TIME); nrDeEsserRef.current.gain.setTargetAtTime(preset.deEsserGain, t, RAMP_TIME);
    }}, []);
  const setOutputVolume = useCallback((volume: number, muted: boolean) => {
    ...
    if (outputGainRef.current && ctx) { outputGainRef.current.gain.setTargetAtTime(next, ctx.currentTime, RAMP_TIME); } else if (outputGainRef.current) outputGainRef.current.gain.value = next;
    ...
    try { const { ctx, source } = getOrCreateAudioSource(audio); ctxRef.current = ctx; sourceRef.current = source;
      connectedAudioRef.current = audio; const nrPreset = NR_PRESETS[noiseReductionMode]; const nyquist = ctx.sampleRate / 2;
      ...
      } else source.connect(nrHighpass); nrHighpass.connect(nrGate); nrGate.connect(nrDeEsser); nrDeEsser.connect(nrDeEssGain); nrDeEssGain.connect(filters[0]);
      ...
      bassShaper.curve = curve; bassShaper.oversample = '2x'; const bassHp = ctx.createBiquadFilter(); bassHp.type = 'highpass';
      bassHp.frequency.value = 80; bassHp.Q.value = 0.7; const bassMix = ctx.createGain(); bassMix.gain.value = bassEnhance;
      ...
      const mbMerge = ctx.createGain(); mbMerge.gain.value = 1.0; const wetAmount = compressorEnabled ? compressorAmount : 0;
      ...
      const mbLowLp = ctx.createBiquadFilter(); mbLowLp.type = 'lowpass'; mbLowLp.frequency.value = 200; mbLowLp.Q.value = 0.7;
      ...
      const mbHighHp = ctx.createBiquadFilter(); mbHighHp.type = 'highpass'; mbHighHp.frequency.value = 3000; mbHighHp.Q.value = 0.7;
      ...
      lastFilter.connect(mbMidBpHp); mbMidBpHp.connect(mbMidBpLp); mbMidBpLp.connect(mbMidComp); mbMidComp.connect(mbWet);
  ...
  const applyPreset = useCallback((gains: number[]) => { setBands(prev => prev.map((b, i) => ({
      ...
      const normGain = normGainRef.current; const nrHead = nrHighpassRef.current; if (source && normalizer && normGain && nrHead) { try {
          ...
          if (next) { normGain.gain.setTargetAtTime(1.6, t, RAMP_TIME); source.connect(normalizer); normalizer.connect(normGain); normGain.connect(nrHead);
  ...
  const toggleCompressor = useCallback(() => { setCompressorEnabled(prev => {
      const next = !prev; saveToStorage(STORAGE_KEYS.COMPRESSOR_ENABLED, next); const ctx = ctxRef.current; const t = ctx?.currentTime ?? 0; const amount = compressorAmount;
      if (mbDryGainRef.current && mbWetGainRef.current) { if (next) {
    ...
    saveToStorage(STORAGE_KEYS.COMPRESSOR_AMOUNT, clamped); if (!compressorEnabled) return; const ctx = ctxRef.current; const t = ctx?.currentTime ?? 0;
