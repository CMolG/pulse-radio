...
function PWAStep() { const [deferredPrompt, setDeferredPrompt] = useState<{ prompt: () => Promise<void> } | null>(null);
  ...
  if (isStandalone) { return ( <div className="flex flex-col items-center gap-4 text-center">
        <IoCheckmarkCircleOutline size={48} className="text-emerald-400" /> <h2 className="text-xl font-bold text-white">Already Installed!</h2>
  ...
  return ( <div className="flex flex-col items-center gap-4 text-center"> <IoPhonePortraitOutline size={48} className="text-[#3478f6]" />
      ...
      {deferredPrompt ? ( <button onClick={handleInstall} className="mt-2 px-6 py-2.5 rounded-xl bg-[#3478f6] text-white font-semibold text-[14px] hover:bg-[#2968d9] transition-colors active:scale-95"
        >Install Now</button>
      ) : isIos ? ( <div className="mt-2 p-3 rounded-xl bg-white/5 border border-white/10"> <div className="flex items-center gap-2 text-[13px] text-white/70">
