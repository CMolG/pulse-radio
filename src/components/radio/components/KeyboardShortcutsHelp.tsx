...
interface Props { onClose: () => void; } export const KeyboardShortcutsHelp = React.memo(function KeyboardShortcutsHelp({ onClose }: Props) { return ( <div
      ...
      aria-modal="true" aria-label="Keyboard shortcuts" onClick={onClose}><div className="bg-surface-2 rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4 relative"
        onClick={(e) => e.stopPropagation()}><div className="flex items-center justify-between mb-4">
          <h2 className="text-[15px] font-semibold text-white">Keyboard Shortcuts</h2><button onClick={onClose} className="p-1 rounded-lg hover:bg-surface-3 transition-colors text-secondary"
            aria-label="Close shortcuts help"><X size={16} /></button></div><div className="space-y-1.5">
          {SHORTCUTS.map(({ key, desc }) => ( <div key={key} className="flex items-center justify-between py-1 px-1"> <span className="text-[13px] text-secondary">{desc}</span>
              <kbd className="text-[12px] font-mono bg-surface-3 text-white px-2 py-0.5 rounded-md min-w-[2rem] text-center"> {key}</kbd></div>))}</div></div></div>
  );});
