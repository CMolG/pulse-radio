...
type HeartAction = { filled: boolean; onClick: () => void; label: string }; type Props = { item: SongCardItem; delay: number; onRemove: () => void; onSelect?: (song: SongDetailData) => void;
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(delay * 0.03, 0.5) }}
      ...
      <div className="p-3 space-y-0.5"><p className="text-[13px] font-medium text-white line-clamp-1">{item.title}</p> <p className="text-[12px] text-secondary line-clamp-1">{item.artist}</p>
        ...
        {(item.genre || item.durationMs) && ( <p className="text-[10px] text-dim line-clamp-1 flex items-center gap-1">
            {item.genre && <span>{item.genre}</span>} {item.durationMs && ( <span className="inline-flex items-center gap-0.5">
