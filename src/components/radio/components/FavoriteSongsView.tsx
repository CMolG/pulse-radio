...
"use client"; import React, { useState, useMemo, useCallback, useEffect, useRef } from "react"; import { createPortal } from "react-dom";
import { Music, Heart, Trash2, Users, X, ChevronDown, Disc3 } from "lucide-react"; import { motion, AnimatePresence } from "motion/react"; import type { FavoriteSong, SongDetailData } from "../types";
...
}; type ContextMenuState = { x: number; y: number; songId: string } | null; type FilterMode = "none" | "artist" | "album"; const filterBtnClass = (active: boolean) =>
...
function SongContextMenu({ menu, onRemove, onClose, }: {
  ...
  const x = Math.min(menu.x, window.innerWidth - menuW - 8); const y = Math.min(menu.y, window.innerHeight - menuH - 8); return createPortal( <div ref={ref} style={{ top: y, left: x, width: menuW }}
      className="fixed z-[200] py-1 rounded-xl bg-surface-3 border border-border-default shadow-2xl backdrop-blur-sm"> <button onClick={() => { onRemove(menu.songId); onClose(); }}
  ...
  return ( <div className="mb-6"> {/* Group header */} <button onClick={() => hasMore && setExpanded(e => !e)} className={`flex items-center gap-2 mb-3 group ${hasMore ? "cursor-pointer" : "cursor-default"}`}>
      ...
      {/* Stacked/expanded cards */} {!expanded && hasMore ? ( <div className="relative cursor-pointer" onClick={() => setExpanded(true)} role="button"
          ...
          {songs.slice(0, VISIBLE_COUNT).map((song, i) => ( <div key={song.id} className="absolute left-0 right-0 transition-all duration-300" style={{ top: `${i * 16}px`, zIndex: VISIBLE_COUNT - i,
            ...
            <span className="text-[11px] text-[#3478f6] font-medium bg-[#3478f6]/10 px-3 py-1 rounded-full border border-[#3478f6]/20"> +{songs.length - VISIBLE_COUNT} more</span></div></div>
              ...
              <div key={song.id} onContextMenu={(e) => { e.preventDefault(); onContextMenu(e, song.id); }}><SongCard
                  item={song} onRemove={() => onRemove(song.id)} onSelect={onSelect} delay={i} heart={null} hideRemove /></div>))}</div></AnimatePresence>
      )} {expanded && hasMore && ( <button onClick={() => setExpanded(false)} className="mt-3 flex items-center gap-1 text-[11px] text-white/40 hover:text-white/60 transition-colors">
...
export default function FavoriteSongsView({ songs, onRemove, onClear, onSelect }: Props) {
  const [filterMode, setFilterMode] = useState<FilterMode>("none"); const [contextMenu, setContextMenu] = useState<ContextMenuState>(null); const handleContextMenu = useCallback((e: React.MouseEvent, songId: string) => {
    ...
    return Array.from(groups.entries()).sort((a, b) => b[1].length - a[1].length);
      ...
      <div className="flex-center-col py-20 px-4"><Heart size={40} className="text-dim mb-3" /> <p className="text-[14px] text-secondary">No favorite songs yet</p>
  ...
  return ( <div className="p-4"><SongContextMenu menu={contextMenu} onRemove={onRemove} onClose={closeContextMenu} /> <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2"><p className="text-[12px] text-dim">{songs.length} songs</p> {/* By Artist */}
          <button onClick={() => toggleFilter("artist")} className={filterBtnClass(filterMode === "artist")}> <Users size={10} /> By Artist
