...
type Props = { song: SongDetailData | null; onClose: () => void; onRemoveFromFavorites?: () => void }; function SongDetailModal({ song, onClose, onRemoveFromFavorites }: Props) {
  ...
  const skeletonWidths = ['w-full', 'w-11/12', 'w-10/12', 'w-9/12', 'w-8/12', 'w-10/12', 'w-7/12']; const lyricsSkeleton = (n: number) => ( <div className="space-y-2 animate-pulse">
  ...
  useEffect(() => { if (!song || !modalRef.current) return; const modal = modalRef.current; const prev = document.activeElement as HTMLElement | null; const focusable = modal.querySelectorAll<HTMLElement>( // Focus first focusable element
      'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
        ...
        'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
      ); if (nodes.length === 0) return; const first = nodes[0]; const last = nodes[nodes.length - 1]; if (e.shiftKey && document.activeElement === first) { e.preventDefault(); last.focus();
          ...
          onClick={onClose}><motion.div key="song-detail-modal" ref={modalRef} role="dialog" aria-modal="true" aria-label={`Song details: ${song.title} by ${song.artist}`} initial={{ y: 30, opacity: 0, scale: 0.96 }}
            ...
            <div className="bg-surface-2 rounded-2xl border border-border-default shadow-2xl w-full max-w-[380px] max-h-[85vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
              {/* Close button */} <div className="sticky top-0 z-10 flex justify-end p-3"><button onClick={onClose} aria-label="Close song details"
              ...
              {/* ── Song Info ── */} <div className="px-5 -mt-2">{/* Artwork */}
                <div className="relative w-full aspect-square max-w-[240px] mx-auto rounded-2xl overflow-hidden bg-surface-3 shadow-xl"> {resolvedArtworkUrl ? ( <UiImage src={resolvedArtworkUrl}
                ...
                {/* Title & artist */} <div className="mt-5 text-center"> <h2 className="text-[17px] font-bold text-white leading-snug line-clamp-2">{song.title}</h2>
                  <p className="text-[14px] text-secondary mt-1">{song.artist}</p> {resolvedAlbum && (
                    ...
                    <div className="mt-2 space-y-1.5"><div className="grid grid-cols-2 items-start"> <div className="justify-self-start">{resolvedDurationMs && (
                      ...
                      {resolvedReleaseDate && ( <p className="text-[10px] text-white/50"> Released on: {formatReleaseDate(resolvedReleaseDate)}</p>
                      )} <div className="flex flex-wrap justify-center gap-1.5"> {resolvedGenre && <MetaBadge icon={Tag} cls="bg-white/[0.06] text-white/50">{resolvedGenre}</MetaBadge>}
              ...
              {/* Divider */} <div className="mx-5 my-5 border-t border-border-default" /> {/* ── Lyrics (mobile) ── */} <div className="px-5 md:hidden">
                <h3 className="text-[11px] font-semibold text-dim uppercase tracking-wider mb-3">Lyrics (plain)</h3> {lyricsLoading && lyricsSkeleton(4)} {!lyricsLoading && plainLyrics && (
