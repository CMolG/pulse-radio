...
export default React.memo(function StatsView({ topStations, topSongs, topArtists, topGenres, totalListenMs }: Props) {
  const hasData = totalListenMs > 0 || topSongs.length > 0; if (!hasData) { return ( <div className="flex flex-col items-center justify-center py-16 px-4">
        <IoTimeOutline size={40} className="text-white/20 mb-3" /> <p className="text-[14px] text-white/40">No listening data yet</p> <p className="text-[12px] text-white/25 mt-1">Start playing stations to see your stats</p></div>
    ); }
  const maxStationTime = topStations[0]?.totalMs ?? 1; const maxSongCount = topSongs[0]?.count ?? 1; const maxArtistCount = topArtists[0]?.count ?? 1; const maxGenreCount = topGenres[0]?.count ?? 1;
  return ( <div className="p-4 space-y-6"> {/* Total listen time */} <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.04] border border-white/8"> <IoTimeOutline size={20} className="text-[#3478f6]" />
        <div><p className="text-[11px] text-white/40 uppercase tracking-wider">Total Listen Time</p> <p className="text-[18px] font-bold text-white tabular-nums">{formatListenTime(totalListenMs)}</p></div></div>
