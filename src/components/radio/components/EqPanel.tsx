...
export default React.memo(function EqPanel({ bands, enabled, normalizerEnabled, stereoWidth, bassEnhance, compressorEnabled, compressorAmount, noiseReductionMode, customPresets = [], onSetGain, onApplyPreset, onToggleEnabled, onToggleNormalizer, onSetStereoWidth, onSetBassEnhance, onToggleCompressor, onSetCompressorAmount, onSetNoiseReductionMode, onClose, onSaveCustomPreset, onRemoveCustomPreset, onPresetChange }: Props) {
  ...
  }; const handleSave = () => { const name = presetName.trim(); if (name && onSaveCustomPreset) { onSaveCustomPreset(name); setPresetName(''); setShowSaveInput(false); } };
  return (<div className="absolute bottom-16 right-4 w-72 bg-sys-surface/95 backdrop-blur-xl border border-border-strong rounded-xl p-4 shadow-2xl z-50">
      {/* Header */} <div className="flex-between mb-4"> <div className="flex-row-2"><span className="text-[13px] font-semibold text-white">Equalizer</span>
 ...
 <button key={preset.name} onClick={() => handleSelectPreset(preset.name, preset.gains)}
            ...
            {preset.name}</button>
        ))} {customPresets.map(preset => ( <div key={`custom-${preset.name}`} className="flex-row-0.5"> <button onClick={() => handleSelectPreset(preset.name, preset.gains)}
 ...
 <input type="range" min={-12} max={12} step={1} value={band.gain} onChange={e => handleSetGain(band.id, parseInt(e.target.value, 10))} disabled={!enabled} aria-label={`${band.label} gain`}
      ...
      {/* Stereo width */} <div className="mt-3 pt-3 border-t border-white/10"> <div className="mb-2"><div className="text-[10px] text-secondary mb-1">Noise Reduction</div>
          <div className="flex-wrap-1.5">{(['off', 'low', 'medium', 'high'] as const).map(mode => ( <button key={mode}
                onClick={() => onSetNoiseReductionMode(mode)} className={`px-2 py-1 text-[10px] rounded-md transition-colors ${ noiseReductionMode === mode
                    ? 'bg-sys-orange/20 text-sys-orange border border-sys-orange/40' : 'bg-surface-2 hover:bg-surface-4 text-secondary hover:text-white'
        ...
        <div className="flex items-center gap-2"><span className="text-[10px] text-secondary shrink-0 w-12">Width</span>
          <input type="range" min={0} max={200} step={5} value={Math.round(stereoWidth * 100)} onChange={e => onSetStereoWidth(parseInt(e.target.value, 10) / 100)} aria-label="Stereo width"
          ...
          <input type="range" min={0} max={100} step={5} value={Math.round(bassEnhance * 100)} onChange={e => onSetBassEnhance(parseInt(e.target.value, 10) / 100)} aria-label="Bass enhance"
            className="flex-1 h-1 appearance-none bg-surface-4 rounded-full cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-2.5 [&::-webkit-slider-thumb]:h-2.5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-sys-orange [&::-webkit-slider-thumb]:shadow-[0_0_4px_rgba(255,159,10,0.4)]"
