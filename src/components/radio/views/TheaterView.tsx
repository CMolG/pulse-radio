...
// Fallback colors — dawn gradient (purple → pink → orange)
const FALLBACK_COLORS: [string, string, string] = ['#8b2fc9', '#c44569', '#f97848'];
...
/** Convert HSL (h: 0-360, s/l: 0-100) to hex string */
...
/** Extract the top 2-4 saturated hues from an artwork image as hex colors. */
  ...
  const p = new Promise<[string, string, string]>((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        resolve([hslToHex(h1, 75, 55), hslToHex(h2, 65, 50), hslToHex(h3, 60, 45)]);
      } catch {
