...
export type LyricsData = { trackName: string; artistName: string; albumName?: string; duration?: number;
  synced: boolean; lines: LyricLine[]; plainText?: string; realtimeCapable?: boolean;
  realtimeLanguage?: 'en' | 'es' | null;
};
export type PlaybackStatus = 'idle' | 'loading' | 'playing' | 'paused' | 'error';
export type PlaybackState = { station: Station | null; status: PlaybackStatus; volume: number; muted: boolean;
...
export type ViewState = { mode: 'top' | 'search' | 'genre' | 'country'; query: string; tag: string; countryCode: string;
  countryQueryName: string; label: string;
};
export type LrcLibResponse = { id: number; trackName: string; artistName: string; albumName: string;
